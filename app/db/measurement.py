...
def create_measurement_for_patient(patient, date, value, cursor=None):
    ...
    """
    cursor.execute('INSERT INTO MEASUREMENT(DATE, VALUE, PATIENT_ID) VALUES (?, ?, ?)',
...
def get_measurements_for_patient_id(patient_id, cursor=None):
    ...
    cursor.execute('SELECT * FROM MEASUREMENT WHERE PATIENT_ID=?',
                   (patient_id,))
    return [Measurement(measurement_id, datetime.strptime(date, DATE_FORMAT), value) for
            measurement_id, date, value, _ in cursor.fetchall()]
...
def delete_measurement_for_patient(patient, measurement, cursor=None):
    ...
    cursor.execute('DELETE FROM MEASUREMENT WHERE ID=?',
                   (measurement.db_id,))
...
def delete_all_measurements_for_patient(patient, cursor=None):
    ...
    cursor.executemany('DELETE FROM MEASUREMENT WHERE ID=?',
                       [(m.db_id,) for m in patient.measurements])

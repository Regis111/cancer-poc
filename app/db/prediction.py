...
def get_predictions_for_patient_id(patient_id: int, cursor=None) -> dict:
    ...
    for creation_date, prediction_value_id, value_date, value in cursor.fetchall():
        prediction = Prediction(prediction_value_id, datetime.strptime(value_date, DATE_FORMAT).date(), value)
        if predictions[creation_date]:
...
def delete_all_predictions_for_patient(patient: Patient, cursor=None):
    ...
    cursor.execute(
        "SELECT P.ID FROM PREDICTION P WHERE P.PATIENT_ID=?",
        (patient.db_id,)
    )
    prediction_ids = cursor.fetchall()
    cursor.executemany(
        "DELETE FROM PREDICTION_VALUE WHERE PREDICTION_ID=?", [(id,) for id in prediction_ids]
    )
    cursor.execute(
        "DELETE FROM PREDICTION WHERE PATIENT_ID=?", (patient.db_id,)
    )

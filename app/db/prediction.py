...
def create_prediction_for_patient(patient, date, value, cursor=None):
    ...
    """
    cursor.execute('INSERT INTO PREDICTION(DATE, VALUE, PATIENT_ID) VALUES (?, ?, ?)',
...
def get_predictions_for_patient_id(patient_id, cursor=None):
    ...
    cursor.execute('SELECT * FROM PREDICTION WHERE PATIENT_ID=?',
                   (patient_id,))
    return [Prediction(prediction_id, datetime.strptime(date, DATE_FORMAT), value) for
            prediction_id, date, value, _ in cursor.fetchall()]
...
def delete_prediction_for_patient(patient, prediction, cursor=None):
    ...
    cursor.execute('DELETE FROM PREDICTION WHERE ID=?',
                   (prediction.db_id,))
...
def delete_all_predictions_for_patient(patient, cursor=None):
    ...
    cursor.executemany('DELETE FROM PREDICTION WHERE ID=?',
                       [(m.db_id,) for m in patient.predictions])

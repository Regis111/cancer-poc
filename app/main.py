...
if __name__ == "__main__":
    ...
    create_measurements_for_patient(
        patient, [(datetime(2020, 8, 10), 21.6), (datetime(2020, 8, 20), 22.6)]
    )
    create_predictions_for_patient(
        patient, [(datetime(2020, 8, 30), 25.1), (datetime(2020, 9, 9), 28.9)]
    )
    create_measurement_for_patient(patient, datetime(2020, 8, 30), 24.7)
    delete_all_predictions_for_patient(patient)
    create_predictions_for_patient(
        patient, [(datetime(2020, 9, 9), 27.4), (datetime(2020, 9, 19), 28.1)]
    )
